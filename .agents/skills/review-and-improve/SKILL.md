...
Restate in a few lines: `README.md`, `AGENTS.md`, `example.env`; `.agents/skills/*/SKILL.md`; `app/`, `agents/`, `teams/`, `workflows/`, `db/`, `evals/`, `scripts/`; `compose.yaml`, `Dockerfile`, `pyproject.toml`, `scripts/aws/task-def.json`. Skip `.venv/`, caches, `.git/`. Fold in any concern the user names.
