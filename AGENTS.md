...
```bash
./scripts/aws/up.sh        # provision ECR + RDS + Secrets Manager + the Express service
./scripts/aws/env-sync.sh  # sync .env.production (default) or a named env file
./scripts/aws/redeploy.sh  # rebuild + push the image, roll the service
./scripts/aws/down.sh      # tear everything down (asks for confirmation; --yes to skip)
```

The heavy lifting is **ECS Express Mode**: one `aws ecs create-express-gateway-service` call provisions the Fargate service, ALB + HTTPS certificate, target groups, security groups, autoscaling, CloudWatch logs and alarms, and a public HTTPS URL (`https://ag-<id>.ecs.<region>.on.aws` — generated per service, not derived from the service name) — up to 25 Express services share one ALB. `up.sh` pins the scaling target to exactly **one task** (min 1 keeps the in-process scheduler alive; max 1 because scaled-out replicas would each run a scheduler, double-firing every cron — the same reason the Fly sibling deploys `--ha=false`). If you ever want scale-out, make the scheduler single-instance first. The URL is generated, so `up.sh` reads the real one from `describe-express-gateway-service` once the service exists and rolls a task-definition revision that bakes it into `AGENTOS_URL`, so the scheduler is reachable in prod out of the box. It also generates `MCP_CONNECT_SECRET` into the env file when missing (stored in Secrets Manager, riding the same correcting revision) and prints it in the closing summary, so chat apps can connect over OAuth from the first deploy — see [MCP interface](#mcp-interface). `up.sh` then waits until the gateway actually answers (first-create ALB + certificate + DNS provisioning runs ~10-25 minutes) and self-heals the known first-run wedge: with freshly created IAM roles, Express's async infrastructure calls can be denied before the role policies propagate, and ECS never retries — the script detects the never-answering endpoint and recreates the service once; `aws ecs monitor-express-gateway-service` is the diagnostic if anything else stalls. Region comes from `AWS_REGION` (default `us-east-1`).

We ship a custom task definition ([`scripts/aws/task-def.json`](scripts/aws/task-def.json)): container named `Main`, port mapping named `main` on 8000, 2 vCPU / 4 GB (Express's own default would be just 0.25 vCPU / 512 MiB), and Secrets Manager injection for every secret-shaped variable — so sizing and secret handling stay ours while Express owns the infrastructure. Around it, `up.sh` provisions the ECR repo and RDS PostgreSQL 17 (db.t4g.micro, private-only — no public address, port 5432 open only inside the VPC); pgvector comes from the app's own `CREATE EXTENSION IF NOT EXISTS` on first boot. The ALB health check hits `/health`, which AgentOS serves unauthenticated by design even in prd, so health checks pass with JWT auth on.

JWT auth is on by default. Once the service URL exists, `up.sh` pauses if `JWT_VERIFICATION_KEY` or `JWT_JWKS_FILE` is missing, so you can connect the OS at os.agno.com (Connect OS → Live with the service URL, name it `Live AgentOS`, and flip Token-Based Authorization (JWT) on right on the connect panel — Settings → OS & Security is the fallback if you connected without it), paste the full PEM into the prompt, and let the script save it to the env file. Live AgentOS Connections are a paid feature; use `PLATFORM30` to get 1 month off. The PEM lands in Secrets Manager and rides into a fresh task-definition revision before traffic is served. If you skip the prompt or run non-interactively, add the key later and run `./scripts/aws/env-sync.sh`.

`up.sh` needs an AWS CLI v2 recent enough to have the Express Mode verbs — it preflights `aws ecs create-express-gateway-service help` and tells you to upgrade (e.g. `brew upgrade awscli`) until that works. This deploy layer fully replaces the old agno-infra approach: no `infra/` directory, no `ag infra up/down` — plain `aws` CLI scripts a coding agent can read, run, and fix mid-failure. Teams that prefer real IaC can drive Express Mode from CloudFormation or Terraform instead.
